    ...
    <div class="single-page-inner">
        <div class="usersec">
            <div class="userDetSec">
                <div class="userNm"><?php echo e(auth()->user()->name); ?></div>
                <div class="userEmail"><?php echo e(auth()->user()->email); ?></div>
            </div>
        </div>
        <hr class="small-marg">
            ...
            <div class="currentPlansDetails">
                <div class="planDetails">
                    <div class="planText">
                        ...
                        <span>
                            <?php if(auth()->user()->subscription_id || auth()->user()->allow_till > now()): ?>
                    ...
                    <div class="billingText">
                        <?php if(auth()->user()->allow_till > now()): ?>
                            Your premium status will remain until: <strong><?php echo e(\Carbon\Carbon::parse(auth()->user()->allow_till)->format("M d, Y")); ?></strong>. You will be able to re-subscribe when your current term is over.
                        <?php elseif(auth()->user()->subscription_id): ?>
                            Your subscription is currently active. Your payment interval is <strong><?php echo e(auth()->user()->checkout->type == 'month' ? 'Monthly' : 'Yearly'); ?></strong>. You will receive further details in your E-mail inbox.
                        <?php else: ?>
                            You are currently not subscribed to any plan.
                        <?php endif; ?>
                    </div>
                </div>
                <?php if(auth()->user()->subscription_id): ?>
                    <a href="<?php echo e(route('cancel.current.subscription.confirmation')); ?>" class="btn custom--btn bg-danger button__md">Cancel Your Subscription</a>
                <?php elseif(auth()->user()->allow_till > now()): ?>
                    <button disabled class="btn custom--btn disabled button__md">Upgrade</button>
                <?php else: ?>
                    <a href="<?php echo e(route('pricing')); ?>" class="btn custom--btn button__md">Upgrade</a>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="currentPlansDetails">
            <div class="planDetails">
                <div class="plansTextBTextArea newBillingEmailPlanText">
                    ...
                    <div class="billingText"><?php echo e(auth()->user()->email); ?></div>
                </div>
            </div>
        </div>
    </div>
...
<?php if(auth()->user()->password != 'NONE'): ?>
    <div class="single-page-sec">
        ...
        <div class="single-page-inner">
            ...
            <?php if(session('resetpassword')): ?>
                <div class="alert alert-success">Your password was reset successfully.</div>
            <?php endif; ?>

            <form method="POST" action="<?php echo e(route('manage-reset-password')); ?>">
                <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.main', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/awsstart/domains/meritcodetechnology.com/public_html/resources/views/auth/manage.blade.php ENDPATH**/ ?>